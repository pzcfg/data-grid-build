...
function useSelectionBehavior(gridSelection, setGridSelection, rangeBehavior, columnBehavior, rowBehavior, rangeSelect) {
  const setCurrent = _react.default.useCallback((value, expand, append, trigger) => {
    ...
    let newVal = {
      ...
      columns: allowColumnCoSelect ? gridSelection.columns : _dataGridTypes.CompactSelection.empty(),
      rows: allowRowCoSelect ? gridSelection.rows : _dataGridTypes.CompactSelection.empty()
    };
  ...
  const setSelectedRows = _react.default.useCallback((newRows, append, allowMixed) => {
    var _newRows;
    ...
    if (rowBehavior === "exclusive" && newRows.length !== 0) {
      ...
      newVal = {
        current,
        columns: columnMixed ? gridSelection.columns : _dataGridTypes.CompactSelection.empty(),
        rows: newRows
      };
    }
  ...
  const setSelectedColumns = _react.default.useCallback((newCols, append, allowMixed) => {
    var _newCols;
    ...
    if (columnBehavior === "exclusive" && newCols.length !== 0) {
      ...
      newVal = {
        current,
        rows: rowMixed ? gridSelection.rows : _dataGridTypes.CompactSelection.empty(),
        columns: newCols
      };
    }
