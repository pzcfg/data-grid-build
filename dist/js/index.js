...
var NumberOverlayEditor = React29.lazy(async () => await import("./number-overlay-editor-7CUBYLZS.js"));
...
var UriOverlayEditor = (p) => {
  ...
  }, preview), !readonly && /* @__PURE__ */ React32.createElement("div", {
    className: "edit-icon",
    onClick: onEditClick
...
var DataEditorImpl = (p, forwardedRef) => {
  ...
  const docStyle = React34.useMemo(() => {
    if (typeof window === "undefined")
      return { fontSize: "16px" };
    return window.getComputedStyle(document.documentElement);
  }, []);
  const fontSizeStr = docStyle.fontSize;
  const remSize = React34.useMemo(() => Number.parseFloat(fontSizeStr), [fontSizeStr]);
  const [rowHeight, headerHeight, groupHeaderHeight, theme, overscrollX, overscrollY] = React34.useMemo(() => {
    var _a2, _b2, _c2;
  ...
  const getMangledCellContent = React34.useCallback(([col, row], forceStrict = false) => {
  ...
  rowsRef.current = rows;
  const appendRow = React34.useCallback(async (col, openOverlay = true) => {
    var _a2;
  ...
  const normalSizeColumn = React34.useCallback(async (col, force = false) => {
    var _a2;
